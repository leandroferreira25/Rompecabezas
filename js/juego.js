...
var instrucciones = ["Mover las fichas", "Ordenalas como se te ocurra", "Probá hasta que logres formar la imagen!"];
...
function guardarMovimiento (movimiento){
  var mostrar = movimientos.push(movimiento);

  actualizarUltimoMovimiento(mostrar);
}
...
function chequearSiGano() {
    ...
    for(var i= 0; i < grilla.length; i++){
      for(var j =0; j < grilla.length; j++){
        ...
        var nuevaGrilla = grilla[i][j];

          if(nuevaGrilla == grilla){
            return true;
          }else{
            return false;
          }
...
function mostrarCartelGanador() {
    //COMPLETAR
    var ganador = chequearSiGano();
    if(ganador == true){
      alert("Te ganaste un 0km");
    }
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    //COMPLETAR
    var auxiliar = grilla[filaPos1][columnaPos1];
